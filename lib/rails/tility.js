var build = function(orig, dest, env){
  ...
  cp(orig + '/lib/rails/application.html.erb', dest + '/app/views/layouts/application.html.erb');

  // init rails in current directory
  if (exec('rails new . --skip-bundle --quiet --skip --database=mysql --template=' + orig + '/lib/rails/template.rb').code !== 0) {
